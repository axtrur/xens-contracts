...
const { expectFailure } = require('./test-utils/exceptions');
...
contract('PublicResolver', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        await ens.setSubnodeOwner('0x0', sha3('ethw'), accounts[0], {from: accounts[0]});
    });
...
function dnsName(name) {
    ...
    buf[offset++] = 0;
    const result =  '0x' + buf.reduce((output, elem) => (output + ('0' + elem.toString(16)).slice(-2)), '');
    return result;
}
