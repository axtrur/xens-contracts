...
contracts.forEach(function ([ENS, lang]) {
    contract('ENS ' + lang, function (accounts) {
        ...
        it('should prohibit transfers by no-owners', async () => {
            await exceptions.expectFailure(
                ens.setOwner('0x0', '0x0000000000000000000000000000000000001234', {from: accounts[1]})
            );
        });
        ...
        it('should prevent setting resolvers by no-owners', async () => {
            await exceptions.expectFailure(
                ens.setResolver('0x0', '0x0000000000000000000000000000000000001234', {from: accounts[1]})
            );
        });
        ...
        it('should prevent setting the TTL by no-owners', async () => {
            await exceptions.expectFailure(ens.setTTL('0x0', 3600, {from: accounts[1]}));
        });
        ...
        it('should prohibit subnode creation by no-owners', async () => {
